...
import { Camera, CreditCard, User, Building2, CheckCircle2, Upload, X } from 'lucide-react';
...
function StepAadhaar({
  ...
  draft: KycDraft;
  onChange: (u: Partial<KycDraft>) => void;
  onValidChange: (v: boolean) => void;
}) {
  const valid =
    !!draft.aadhaar_photo_url &&
    /^\d{4}$/.test(draft.aadhaar_last_4);

  // Notify parent on each render
  if (typeof window !== 'undefined') {
    setTimeout(() => onValidChange(valid), 0);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PhotoUploader
        ...
        value={draft.aadhaar_photo_url}
        onChange={url => onChange({ aadhaar_photo_url: url })}
      />

      <div className="space-y-1">
        <label className="text-sm font-semibold text-ink" htmlFor="aadhaar_last_4">
        ...
        <input
          ...
          value={draft.aadhaar_last_4}
          onChange={e => {
            const v = e.target.value.replace(/\D/g, '').slice(0, 4);
            onChange({ aadhaar_last_4: v });
          }}
  ...
  return (
    <div className="space-y-6">
      ...
      <PhotoUploader
        ...
        hint="Clear photo showing the PAN number"
        value={draft.pan_photo_url}
        onChange={url => onChange({ pan_photo_url: url })}
      />

      <div className="space-y-1">
        <label className="text-sm font-semibold text-ink" htmlFor="pan_number">
          PAN number
        </label>
        <input
          id="pan_number"
...
function StepSelfie({
  ...
  return (
    <div className="space-y-6">
      ...
      <PhotoUploader
        label="Your selfie"
        hint="Use the front camera"
        value={draft.selfie_url}
        onChange={url => onChange({ selfie_url: url })}
        capture="user"
      />
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex rounded-2xl overflow-hidden border border-gray-200">
        ...
        <button
          type="button"
          onClick={() => onChange({ payment_method: 'upi' })}
          className={cn(
            'flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold transition-colors',
...
function StepReview({
  ...
  if (typeof window !== 'undefined') {
    setTimeout(() => onValidChange(true), 0);
  }
...
export default function LenderKycPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push('/lender/dashboard?kyc=submitted');
    } catch {
  ...
  return (
    <main className="min-h-screen flex flex-col px-6 py-10">
      ...
      <div className="flex-1">
        {step === 1 && (
          <StepAadhaar draft={draft} onChange={updateDraft} onValidChange={setStepValid} />
        )}
        {step === 2 && (
          <StepPan draft={draft} onChange={updateDraft} onValidChange={setStepValid} />
        )}
        {step === 3 && (
          <StepSelfie draft={draft} onChange={updateDraft} onValidChange={setStepValid} />
        )}
        {step === 4 && (
          <StepBankUpi draft={draft} onChange={updateDraft} onValidChange={setStepValid} />
        )}
        {step === 5 && (
